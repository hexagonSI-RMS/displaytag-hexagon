...
public class HeaderCell
{
	...
	public boolean getIsGroupedHeader() {
		return isGroupedHeader;
	}

	public void setIsGroupedHeader(boolean isGroupedHeader) {
		this.isGroupedHeader = isGroupedHeader;
	}
	...
	public void setColspanSize(int colspanSize) {
		this.colspanSize = colspanSize;
	}

	public void addColSpan(int colSpanCount) {
		if ((colSpanCount == 0 )||(colSpanCount == 1)){
			...
			Object colspanAttributes = this.headerAttributes.get(TagConstants.ATTRIBUTE_COL_SPAN);
	
			// handle multiple values
			if (colspanAttributes == null) {
				this.headerAttributes.put(TagConstants.ATTRIBUTE_COL_SPAN, new MultipleHtmlAttribute(String.valueOf(colSpanCount)));
		    }
		}
	}
	    
	public void addRowSpan(int rowSpanCount){
		if ((rowSpanCount == 0 )||(rowSpanCount == 1)){
			return;    		
		} else {
			// if headerAttributes has not been set, instantiates a new map
			if (headerAttributes == null) {
			...
			Object colspanAttributes = this.headerAttributes.get(TagConstants.ATTRIBUTE_ROW_SPAN);
	
			// handle multiple values
			if (colspanAttributes == null) {
				this.headerAttributes.put(TagConstants.ATTRIBUTE_ROW_SPAN, new MultipleHtmlAttribute(String.valueOf(rowSpanCount)));
			}
		}
	}
    /****  DISPL-242  ********/
