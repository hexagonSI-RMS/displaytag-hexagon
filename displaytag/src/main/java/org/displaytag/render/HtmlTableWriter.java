...
public class HtmlTableWriter extends TableWriterAdapter
{
    ...
    {
        // DISPL-242: handle new grouped header title if necessary
        if (model.isGroupedTableHeader()) {
        	writeGroupedTableHeader(model);
        } else {
        	writeRegularTableHeader(model);
        }
    }

    /**
     * Writes "regular" table's column header.
     */
    private void writeRegularTableHeader(TableModel model)
    {
        if (log.isDebugEnabled())
        {
            log.debug("[" + tableModel.getId() + "] writeRegularTableHeader called");
        }
    ...
    /**
     * DISPL-242: Writes "grouped" column header of a table.
     */
    private void writeGroupedTableHeader(TableModel model)
    {
   	  if (log.isDebugEnabled())
      {
          log.debug("[" + tableModel.getId() + "] writeGroupedTableHeader called");
      }
      ...
      /*** writing down the main header ***/

      // iterator on columns for header
      Iterator iterator = this.tableModel.getMainHeaderCellList().iterator();
      while (iterator.hasNext())
      {
          // get the header cell
          HeaderCell headerCell = (HeaderCell) iterator.next();
          // log.debug("header cell"+headerCell.toString());

          if (headerCell.getIsGroupedHeader())
          {

          }
          else
          {
           	  //as its a grouped header
           	  headerCell.addRowSpan(2); 	
	          if (headerCell.getSortable())
	          {
	              String cssSortable = this.properties.getCssSortable();
	              headerCell.addHeaderClass(cssSortable);
          ...
          write(header);
          write(headerCell.getHeaderCloseTag());
          // System.out.print("header "+headerCell.toString());
      }

      // close tr
      write(TagConstants.TAG_TR_CLOSE);

      /*** end of writing the first/main row ***/

      /*** write second row of the group header using subHeader list ***/
      // open tr
      write(TagConstants.TAG_TR_OPEN);

      Iterator subHeaderItr = this.tableModel.getSubHeaderCellList().iterator();
      while (subHeaderItr.hasNext())
      {
          // get the header cell
          HeaderCell subheaderCell = (HeaderCell) subHeaderItr.next();

          if (subheaderCell.getSortable())
          {
              String cssSortable = this.properties.getCssSortable();
              subheaderCell.addHeaderClass(cssSortable);
          }

          // if sorted add styles
          if (subheaderCell.isAlreadySorted())
          {
              // sorted css class
           	  subheaderCell.addHeaderClass(this.properties.getCssSorted());

              // sort order css class
           	  subheaderCell.addHeaderClass(this.properties.getCssOrder(this.tableModel.isSortOrderAscending()));
          }

          // append th with html attributes
          write(subheaderCell.getHeaderOpenTag());

          // title
          String header = subheaderCell.getTitle();

          // column is sortable, create link
          if (subheaderCell.getSortable())
          {
              // creates the link for sorting
              Anchor anchor = new Anchor(getSortingHref(subheaderCell), header);
          ...
          write(header);
          write(subheaderCell.getHeaderCloseTag());
          // log.debug("sub header cell "+subheaderCell.toString());
      }

      // close tr
      write(TagConstants.TAG_TR_CLOSE);

      /*** end of writing the second row ***/
      ...
      write(TagConstants.TAG_THEAD_CLOSE);
   	  ...
   	  if (log.isDebugEnabled())
      {
          log.debug("[" + tableModel.getId() + "] writeGroupedTableHeader end");
      }
    }
